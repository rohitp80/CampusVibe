...
async function setupTables() {
  try {
    ...
    const { error: friendRequestsError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    if (friendRequestsError) {
      console.log('Friend requests table might already exist or using direct creation...');
    }
    
    // Create friendships table
    console.log('Creating friendships table...');
    const { error: friendshipsError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    if (friendshipsError) {
      console.log('Friendships table might already exist or using direct creation...');
    }
    
    console.log('Tables setup completed! Please run the remaining SQL in Supabase SQL editor:');
    console.log(`
-- Enable RLS
